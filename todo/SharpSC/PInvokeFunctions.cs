...
namespace SharpSC
{
    class PInvokeFunctions
    {
        ...
        public static void InstallService(string hostname, string ServiceName, string ServiceDisplayName, string ServiceEXE)
        {
            try
            {
            ...
            using (var scmHandle = OpenSCManager(hostname, null, SCM_ACCESS.SC_MANAGER_CREATE_SERVICE))
            {
                ...
                {
                    Console.WriteLine("[-] Error retrieving a handle to the ServiceControlManager. Reason: Invalid Handle.");
                    // Environment.Exit(1);
                }
                ...
                using (
                    var serviceHandle = CreateService(
                        ...
                        SERVICE_TYPES.SERVICE_WIN32_OWN_PROCESS,
                        SERVICE_START_TYPES.SERVICE_AUTO_START,
                        ...
                        null))
                {
                    ...
                    {
                        Console.WriteLine("[-] Error creating service: received an invalid handle.");
                        // Environment.Exit(1);
                    }

                    Console.WriteLine("[*] Created {0} Service on {1}", ServiceName, hostname);
                }
            }
        }
        public static void UninstallService(string hostname, string ServiceName)
        {
            using (var scmHandle = OpenSCManager(hostname, null, SCM_ACCESS.SC_MANAGER_CREATE_SERVICE))
            {
                if (scmHandle.IsInvalid)
                {
                    Console.WriteLine("[-] Error uninstalling {0} on {1}. Reason: Invalid Handle.", ServiceName, hostname);
                } else
                {
                    using (var serviceHandle = OpenService(scmHandle, ServiceName, SERVICE_ACCESS.SERVICE_ALL_ACCESS))
                    {
                        if (serviceHandle.IsInvalid)
                        {
