...
public static string Execute(string commandLine)
{
    ...
    return sw.ToString();
        ...
        static void Main(string[] args)
        {
            ...
            string usage = "[-] Usage:\n" +
                "  EncryptedZIP.exe encrypt <path to compress> <encryption key>\n" +
                "  EncryptedZIP.exe decrypt <path to encrypted ZIP> <encryption key>";
            ...
            if (args[0].ToLower() == "encrypt")
            {
                ...
                FileAttributes pathAttr = File.GetAttributes(args[1]);
                ...
                else
                {
                    ...
                    AddToArchive(archiveName, args[1]);
                }
            ...
            else if (args[0].ToLower() == "decrypt")
            {
                ...
                string[] fileName = (Path.GetFileNameWithoutExtension(args[1])).Split('.');
                string outFile = fileName[0] + ".zip";
                try
                {
                    Decrypter(args[1], outFile, args[2]);
